...
const SubmittedData = () => {
  ...
  const transformData = (data) => {
    ...
    console.log('Dates of the transformedData:', dates)
    ...
    // Log shares for each member
    for (let member in rows) {
      console.log(`Shares for ${member}:`)
      for (let date in rows[member].shares) {
        console.log(`Date: ${date}, Share value: ${rows[member].shares[date]}`)
      }
    }
  ...
  const columns = [
    ...
    ...dates.map((date) => ({
      ...
      renderCell: (params) => {
        ...
        const { row, field: date } = params
        console.log(`Date: ${date}`)
        const dateShareValue = row.shares[date] ? row.shares[date] : 0
        console.log(`Share value: ${dateShareValue}`)
...
//         console.log('doc data:', docData) // Log the data of each document
//         return docData.rows.map((row, index) => {
//           const joinDate = row.joinDate ? new Date(row.joinDate).toLocaleDateString() : 'N/A' // Ensure valid date
//           console.log('Processed joinDate:', joinDate)
//           return {
//             id: `${doc.id}-${index}`, // Unique id for each row
//             number: index + 1, // Add a 'number' field to each object
//             memberName: Array.isArray(row.memberNames)
...
//   const columns = [
//     { field: 'id', headerName: 'N°', width: 70 },
//     { field: 'memberName', headerName: 'Member Names', width: 200 },
//     ...dates.map((date) => ({
//       field: date,
//       headerName: date,
//       width: 130,
...
//               }}
//             >
//               <BounceLoader color="#00BFFF" loading={meetingsLoading} size={150} />
//             </div>
//           ) : (
